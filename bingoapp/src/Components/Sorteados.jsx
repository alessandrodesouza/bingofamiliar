  ...
  renderizarNumeros = () => {
    ...
    for (let n = this.min; n <= this.max; n++) {
      if (this.props.numerosSorteados)
        numeros.push(
          <span key={n.toString()} className={(this.props.numerosSorteados.indexOf(n) >= 0 ? "text-success animated flash" : "texto-branco")}>
          {(n.toString().length === 1 ? "0" : "") + n.toString()} &nbsp;
          </span>
        );
      else
        numeros.push(
        <span key={n.toString()} className="texto-branco"> {(n.toString().length === 1 ? "0" : "") + n.toString()} &nbsp;</span>
        );
